...
for epoch in range(start_epoch, EPOCHS):
    model.train()
    progress_bar = tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{EPOCHS}", initial=global_step % len(train_dataloader))
    ...
    for center_words, context_words, neg_samples in progress_bar:
        ...
        if global_step % EVALUATION_STEP == 0:
            ...
            with torch.no_grad():
                for val_center, val_context, val_neg in valid_dataloader:
                    val_center, val_context, val_neg = val_center.to(device), val_context.to(device), val_neg.to(device)
            ...
            if avg_val_loss < best_validation_loss:
                ...
                patience_counter = 0
                print(f"New best validation loss. Saving best model to '{BEST_MODEL_FILE}'...")
        ...
        if global_step % CHECKPOINT_STEP == 0:
            ...
            torch.save({
                ...
                'best_validation_loss': best_validation_loss, 'patience_counter': patience_counter
            }, CHECKPOINT_FILE)
...
if os.path.exists(BEST_MODEL_FILE):
    print(f"Loading best model from '{BEST_MODEL_FILE}' for final embedding extraction...")
    best_model_cp = torch.load(BEST_MODEL_FILE)
    model.load_state_dict(best_model_cp['model_state_dict'])

embeddings = model.in_embed.weight.cpu().data

# Save in the format expected by tester.py
checkpoint_data = {
...
# Save with default filename that tester.py expects
torch.save(checkpoint_data, 'cbow_text8.pt')

# Also save individual files for other potential uses
...
print("File 'cbow_text8.pt' created in the format expected by tester.py.")
print("Individual files also saved for compatibility.")
